...
# fit_lasso <- glmnet(X_lasso, y_lasso, alpha = 1, lambda = grid.lambda)
# # make predictions
# y_lasso_hat <- predict(fit_lasso, X_lasso, type="response")
# # summarize accuracy
# mse_lasso <- colMeans((y_lasso - y_lasso_hat)^2)

X_lasso = model.matrix(SalePrice ~.,housing_cleaned)[,-1]
XMorty = X_lasso[nrow(X_lasso), , drop = FALSE]
X_lasso = X_lasso[-nrow(housing_cleaned),]
...
X <- X[,col_names]
XMorty = XMorty[,col_names, drop = FALSE]

myDf <- data.frame(y = y, X)
#Fitting OLS Model
ols.fit = lm(y~., data=myDf)
ols.fit$coefficients

Y_pred_Morty = predict(ols.fit, as.data.frame(XMorty), interval="confidence", level = 0.90)
